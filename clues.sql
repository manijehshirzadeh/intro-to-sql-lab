...
-- Write SQL query here
SELECT countrycode, percentage FROM countrylanguages WHERE language = 'Italian' AND percentage = 100.0;
...
-- Write SQL query here
SELECT name FROM cities WHERE name = (SELECT countries.name FROM countries WHERE name = countries.name);

SELECT cities.name, cities.countrycode
FROM cities
INNER JOIN countries ON cities.name=countries.name;
...
-- Write SQL query here
SELECT name  FROM cities WHERE name LIKE 'Serra%' LIMIT 1;
